...
#for x in ['annotatedTrump2.csv']:
    with codecs.open('../python/annotated2/annotated.csv.sorteda' + x, 'rb') as csvfile:
...
labeled_words = ([(word, 'hillary') for word in hil_tweets] + [(word, 'trump') for word in trump_tweets])
...
i = 1
with open("hitroutput.csv", 'wb') as f:
    for tweet in test_set:
        op1 = purifyText(tweet[13])
        result = classifier.classify(tweet_word(op1))
        op = getEntities(op1)
        if "trump" in op and result == "hillary":
            print tweet[13]
            print "Hillary Positive - Trump Negative"
        elif "hillary" in op and result == "trump":
            print tweet[13]
            print "Trump Positive - Hillary Negative"
        #else:
